...
describe('App832 redesign', () => {
  ...
  describe('local-only favorites (localStorage)', () => {
    it('persists a followed calendar to localStorage and never calls a favorites API', async () => {
      const { container } = render(<App />)
      await waitFor(() => expect(screen.getByText('Neumos')).toBeInTheDocument())
      fireEvent.click(container.querySelector('.pill-follow'))
      await waitFor(() => {
        const stored = JSON.parse(localStorage.getItem('calendar-ripper-favorites') || '[]')
        expect(stored).toContain('test-ripper-cal1.ics')
      })
      // No backend calls for auth or list/favorites sync.
    ...
    it('adds a saved search and persists it to localStorage', async () => {
      render(<App />)
      await waitDiscover()
      clickNav('You')
      await waitFor(() => expect(screen.getByText('Saved searches')).toBeInTheDocument())
      fireEvent.click(screen.getByText('Add search').closest('button'))
      const input = await screen.findByPlaceholderText(/Search term/)
      fireEvent.change(input, { target: { value: 'jazz' } })
      fireEvent.click(screen.getByText('Add'))
      await waitFor(() => {
        const stored = JSON.parse(localStorage.getItem('calendar-ripper-search-filters') || '[]')
        expect(stored).toContain('jazz')
      })
    })
  })
})

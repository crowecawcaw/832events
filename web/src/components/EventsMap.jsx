...
function EventsMapInner({
  ...
  // that first render is culled to the clamp box the map always opens framed at
  // (INITIAL_BOUNDS), so far-flung out-of-county groups never enter the initial
  // marker build — they appear as soon as the real viewport includes them
  // (ViewportTracker seeds actual bounds right after mount).
  // See docs/web-tab-switch-performance.md, "cheap first marker render".
  ...
  // container + tiles commit and paint first, then the (potentially
  // thousands-strong) marker build renders in an interruptible transition. That
  // is what a user perceives as "the map opened" — tiles first, pins a beat
  // later — and it keeps the tap that opened the map responsive.
  // See docs/web-tab-switch-performance.md, "cheap first marker render".

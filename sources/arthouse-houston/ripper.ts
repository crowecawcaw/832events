/**
 * Ripper for Arthouse Houston (https://arthousehtx.eventive.org/schedule)
 *
 * Arthouse Houston is a 501(c)(3) nonprofit dedicated to enriching Houston's
 * arthouse cinema community through film programming at multiple venues
 * (River Oaks Theatre, Majestic Metro, and Garden Oaks Theater).
 *
 * The website uses Eventive ticketing platform which requires JavaScript
 * rendering (proxy access via Browserbase). This ripper extracts event data
 * from the rendered HTML, parsing:
 *   - Event title (film title + venue/program name)
 *   - Date and time
...
const ORGANIZATION_NAME = "Arthouse Houston";
 ...
 * Looks for common Eventive patterns and event containers.
 */
function extractEventElements(html: string): HTMLElement[] {
    const root = parse(html);
    const events: HTMLElement[] = [];

    // Pattern 1: Look for event cards/items with show or event data
    let items = root.querySelectorAll(
        "[data-eventid], [class*='event'], [class*='show'], [class*='card'], .e-show-item, [data-show]"
    );
    if (items.length > 0) {
        events.push(...items);
    }

    // Pattern 2: Generic article/div containers with event-like data
    if (events.length === 0) {
        items = root.querySelectorAll("article, li, [role='listitem']");
        if (items.length > 0) {
            events.push(...items);
        }
    }
    ...
    for (const el of events) {
        const key = el.textContent?.substring(0, 100) || "";
        if (!seen.has(key)) {
            seen.add(key);
            unique.push(el);
        }
...
function parseEventElement(
    ...
    try {
        ...
        const titleEl = element.querySelector("h2, h3, h4, .title, [class*='title'], a");
        if (titleEl) {
        ...
        // Skip certain text patterns that aren't event titles
        if (title.match(/^(show|event|schedule|memberships?)$/i) || title.length < 2) {
            return {
                type: "ParseError" as const,
                reason: "Skipped non-event text",
                context: title,
            };
        }
        ...
        const dateEl = element.querySelector(".date, [class*='date'], time, [class*='time']");
        if (dateEl) {
            dateStr = dateEl.textContent?.trim() || dateEl.getAttribute("datetime") || null;
        }

        // If no date found, try to find any text that looks like a date in the element
        if (!dateStr) {
        ...
        // Try to find time in the text (e.g., "7:00 PM")
        if (!timeStr) {
            const text = element.textContent || "";
            const timeMatch = text.match(/(\d{1,2}):(\d{2})\s*(?:am|pm)/i);
            timeStr = timeMatch ? timeMatch[0] : null;
        }
...
export default class ArthouseHoustonRipper implements IRipper {
    public async rip(ripper: Ripper): Promise<RipperCalendar[]> {
        ...
        if (!res.ok) {
            throw new Error(
                `Arthouse Houston fetch failed: HTTP ${res.status} ${res.statusText}`,
            );
        }
        ...
        if (eventElements.length === 0) {
            errors.push({
                ...
                context: "Check if Eventive widget rendered correctly",
            });
        }
